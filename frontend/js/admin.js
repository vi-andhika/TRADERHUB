...
function loadOrders() {
  ...
  document.getElementById("pendingOrders").textContent = sampleOrders.filter(
    o => o.status === "pending",
  ).length;
}
...
function loadTransactions() {
  ...
  sampleTransactions.forEach(transaction => {
    const row = document.createElement("tr");
    const statusBadge =
      transaction.status === "success" ? "badge-success" : "badge-warning";
    row.innerHTML = `
...
function updateOrderStatus(orderId) {
  const status = prompt(
    "Masukkan status baru (pending/processing/shipped/delivered):",
  );
  if (status) {
...
function renderSalesChart() {
  ...
  // Data penjualan per hari (7 hari terakhir)
  const salesData = {
    Senin: 2500000,
    Selasa: 3200000,
    Rabu: 4100000,
  ...
  // Hitung total penjualan untuk context
      ...
      datasets: [
        {
          label: "Penjualan (Rp)",
          data: data,
          borderColor: "rgb(94, 73, 255)",
          backgroundColor: "rgba(94, 73, 255, 0.1)",
          borderWidth: 3,
          fill: true,
          tension: 0.4,
          pointRadius: 6,
          pointBackgroundColor: "rgb(94, 73, 255)",
